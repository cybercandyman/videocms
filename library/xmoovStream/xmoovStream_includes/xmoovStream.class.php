...
class xmoovStream
{
	...
	function init($start=false)
	{
		...
		&&(isset($this->_config['use_error_handling']) || isset($this->_config['use_activity_handling'])) 
		&&($this->_config['use_error_handling'] || $this->_config['use_activity_handling'])) 
		$this->xsLog=new xsLog();		
		
		# start download
		if (isset($this->_config['file']))
		{
			if($this->set_file($this->_config['file']))
			{
	...
	{			
		...
		{				
			...
			if(is_array($config)) $this->_config=array_merge($this->_config, $config);			
		}
	}
	...
	function header()
	{
		...
		if(isset($this->_config['force_download']) && $this->_config['force_download'])
		{
	...
	function download()
	{
		...
		if(isset($this->_config['position'])) $this->seek_start=intval($this->_config['position']);
		if(isset($this->_config['buffer_size'])) $this->buffer_size=intval($this->_config['buffer_size']) * 1024;
		...
		if(isset($this->_config['use_http_range']) && $this->_config['use_http_range'])
		{
			$this->http_range();
		...
		$bytes_sent=0;
		$chunk=1;
		$throttle=isset($this->_config['throttle']) ? $this->_config['throttle'] : false;
		$burst=isset($this->_config['burst_size']) ? $this->_config['burst_size'] * 1024 : 0;
	...
	function set_file($file)
	{
		...
		if(isset($this->_config['file_path']) && is_dir($this->_config['file_path']))
		{
			$file=$this->_config['file_path'].$file;
		}
		else
		{
		...
		{
			$this->file_name=isset($this->_config['output_file_name']) ? $this->_config['output_file_name'] : basename($file);
	...
	function get_mime_type($file=null)
	{
		...
		return(isset($this->_config['mime_types'][$ext])) ? $this->_config['mime_types'][$ext] : false;
	}
