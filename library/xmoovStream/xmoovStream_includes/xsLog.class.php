...
class xsLog
{
    ...
    function log($event_type=null,$event=null,$conf=null)
    {
		...
		$event_file = isset($this->_config['file']) ? 'file: (' . $this->_config['file'] . ')' : '';
		$show_event = false;
		$notify_event = false;
		$log_event = false;
			...
			case 'error' :
				switch($event)
				{
					...
					case 'file_empty' :
						...
						break;	
					case 'file_path' :
						$event_string = ('Server path does not exist ('.$this->_config['file_path'].')'); 
						break;		
					case '404' :
				...
				if(isset($this->_config['show_errors'])) $show_event = $this->do_output ($this->_config['show_errors'],$event);		
				if(isset($this->_config['log_errors'])) $log_event = $this->do_output ($this->_config['log_errors'],$event);			
				if(isset($this->_config['notify_email']) && isset ($this->_config['notify_errors'])) $notify_event = $this->do_output ($this->_config['notify_errors'],$event);	
				break;
			case 'activity' :
				if(isset($this->_config['log_activity'])) $log_event = $this->do_output ($this->_config['log_activity'],$event);
				$event_string = $event;
				break;
		}
	...
	{	
		if ($this->_config['notify_email']) mail($this->_config['notify_email'],' xmoovStream ' . $subject,$message,'From: noreply@'.$_SERVER['HTTP_HOST'].'\r\n' . 'Reply-To: noreply@'.$_SERVER['HTTP_HOST']."\r\n" . 'X-Mailer: PHP/'.phpversion());
	}
