...
(function($) {
			...
			$(this).addClass("treeTable").find("tbody tr").each(function() {
				// Initialize root nodes only if possible
					if (!options.expandable
						...
						// To optimize performance of indentation, I retrieve
						// the padding-left
						// value of the first root node. This way I only have to
						// call +css+
						// once.
						if (isNaN(defaultPaddingLeft)) {
							defaultPaddingLeft = parseInt($(
									$(this).children("td")[options.treeColumn])
									.css('padding-left'), 10);
						}
	...
	$.fn.collapse = function() {
		...
		childrenOf($(this)).each(function() {
			...
			this.style.display = "none"; // Performance! $(this).hide() is
											// slow...
			});
	...
	$.fn.expand = function() {
		...
		childrenOf($(this)).each(function() {
			...
			// this.style.display = "table-row"; // Unfortunately this is not
			// possible with IE :-(
				$(this).show();
			});
		return this;
	...
	$.fn.createBranch = function(content) {
		var node = $(this);
		node.append(content);
	}
	...
	$.fn.deleteBranch = function() {
		var node = $(this);
		childrenOf(node).css("display", "none");
