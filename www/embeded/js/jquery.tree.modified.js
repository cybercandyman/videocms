...
(function($) {
	...
	$.tree = {
		...
		drag_drop : {
			...
			mouseup : function(event) {
				...
				// RESET EVERYTHING
				if($.tree.drag_drop.marker) $.tree.drag_drop.marker.hide();
	...
	function tree_component () {
		return {
			...
			init : function(elem, conf) {
				...
				if($.tree.drag_drop.marker === false) $.tree.drag_drop.marker = $("<div>").attr({ id : "jstree-marker" }).hide().appendTo("body");
			...
			attach_events : function () {
				...
				this.container
					.bind("mousedown.jstree", function (event) {
						if($.tree.drag_drop.isdown) {
							$.tree.drag_drop.move_type = false;
							event.preventDefault();
							event.stopPropagation();
				...
				$("#" + this.container.attr("id") + " li a")
					...
					.live("mousedown.jstree", function (event) {
						...
						if(_this.settings.rules.multiple != false && _this.selected_arr.length > 1 && obj.children("a:eq(0)").hasClass("clicked")) {
							...
							for(var i in _this.selected_arr) {
								...
								if(_this.check("draggable", _this.selected_arr[i])) {
									...
									$.tree.drag_drop.origin_tree = _this;
									counter ++;
								}
							}
							if(counter > 0) {
								if(_this.check("draggable", obj))	$.tree.drag_drop.drag_node = obj;
								else								$.tree.drag_drop.drag_node = _this.container.find("li.dragged:eq(0)");
								$.tree.drag_drop.isdown		= true;
								$.tree.drag_drop.drag_help	= $("<div id='jstree-dragged' class='tree " + ( _this.settings.ui.theme_name != "" ? " tree-" + _this.settings.ui.theme_name : "" ) + "' />").append("<ul class='" + _this.container.children("ul:eq(0)").get(0).className + "' />");
								var tmp = $.tree.drag_drop.drag_node.clone();
								if(_this.settings.languages.length > 0) tmp.find("a").not("." + _this.current_lang).hide();
								$.tree.drag_drop.drag_help.children("ul:eq(0)").append(tmp);
								$.tree.drag_drop.drag_help.find("li:eq(0)").removeClass("last").addClass("last").children("a").html("<ins>&nbsp;</ins>Multiple selection").end().children("ul").remove();

								$.tree.drag_drop.dragged = _this.container.find("li.dragged");
							}
						}
						else {
							if(_this.check("draggable", obj)) {
								$.tree.drag_drop.drag_node	= obj;
								$.tree.drag_drop.drag_help	= $("<div id='jstree-dragged' class='tree " + ( _this.settings.ui.theme_name != "" ? " tree-" + _this.settings.ui.theme_name : "" ) + "' />").append("<ul class='" + _this.container.children("ul:eq(0)").get(0).className + "' />");
								var tmp = obj.clone();
								if(_this.settings.languages.length > 0) tmp.find("a").not("." + _this.current_lang).hide();
								$.tree.drag_drop.drag_help.children("ul:eq(0)").append(tmp);
								$.tree.drag_drop.drag_help.find("li:eq(0)").removeClass("last").addClass("last");
								$.tree.drag_drop.isdown		= true;
								$.tree.drag_drop.foreign	= false;
								$.tree.drag_drop.origin_tree = _this;
						...
						$.tree.drag_drop.init_x = event.pageX;
						$.tree.drag_drop.init_y = event.pageY;
						obj.blur();
						event.preventDefault(); 
						event.stopPropagation();
						return false;
			...
			scroll_check : function (x,y) { 
				...
				if(cnt.scrollLeft() != sl || cnt.scrollTop() != st) {
					$.tree.drag_drop.move_type	= false;
					$.tree.drag_drop.ref_node	= false;
					$.tree.drag_drop.marker.hide();
				}
				$.tree.drag_drop.scroll_time = setTimeout( function() { _this.scroll_check(x,y); }, 50);
			},
			...
			check_move : function (nod, ref_node, how) {
				...
				if(this.settings.rules.use_max_children) {
					if(typeof r.max_children != "undefined" && r.max_children != -1) {
						...
						var c_count = 1;
						if($.tree.drag_drop.moving == true && $.tree.drag_drop.foreign == false) {
							c_count = $.tree.drag_drop.dragged.size();
	...
	tree_component.mousemove = function(event) {
		...
		if(tmp.isdown) {
			...
			$.tree.drag_helper = tmp.drag_help;
			...
			if(event.target.tagName == "A" || event.target.tagName == "INS") {
				...
				if(ok) {
					switch(mov) {
						case "before":
							goTo.y = et_off.top - 2;
							$.tree.drag_drop.marker.attr("class","marker");
							break;
						case "after":
			...
			if( (et.is(".tree") || et.is("ul") ) && et.find("li:eq(0)").size() == 0) {
				...
				if(tmp.drag_help !== false) tmp.drag_help.find(".forbidden").removeClass("forbidden");
				$.tree.drag_drop.marker.attr("class","marker_plus");
				$.tree.drag_drop.marker.css({ "left" : (et_off.left + 10) , "top" : et_off.top + 15 }).show();
			}
	...
	$(function () { 
		...
		$(document).bind("mouseup.jstree",		$.tree.mouseup); 
	});
